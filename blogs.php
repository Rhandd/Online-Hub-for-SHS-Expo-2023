    ...
    <section class="container my-3">
        <div class="row">
            ...
            <div class="col-12 main">
                ...
                $sql = "SELECT post_id, post_name, post_desc, post_date, post_img, ur.id, ur.uname, ct.category_id, ct.category_name
                FROM posts
                LEFT JOIN users ur ON posts.author=ur.id
                LEFT JOIN category ct ON posts.category=ct.category_id
                WHERE posts.status = 'approved'
            ...
            if ($respg->num_rows() > 0) {
                ...
                if ($pageno > 1) {
                    echo '<li class="paginationli prev"><a href="blogs.php?page=' . ($pageno - 1) . '">Prev</a></li>';
                } else {
                    $pageno = 1;
                    echo '<li class="paginationli prev"><a href="blogs.php?page=' . ($pageno) . '">Prev</a></li>';
                }
                for ($i = 1; $i <= $ttlpages; $i++) {
                    if ($pageno == $i) {
                        $pgactive = "pgactive";
                    }
                    echo '<li class="paginationli ' . '"><a href="blogs.php?page=' . $i . '">' . $i . '</a></li>';
                }
                if ($pageno < $ttlpages) {
                    echo '<li class="paginationli next"><a href="blogs.php?page=' . ($pageno + 1) . '">Next</a></li>';
                } else {
                    $pageno = $ttlpages;
                    echo '<li class="paginationli prev"><a href="blogs.php?page=' . ($pageno) . '">Next</a></li>';
                }
                echo '</ul>';
            }
            ?>
